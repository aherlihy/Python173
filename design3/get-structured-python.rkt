  ...
  (match pyjson
    ...
    [(hash-table ('nodetype "ExceptHandler")
     ...
     (PyExcept (if (equal? type #\nul) (PyId (string->symbol "ExceptAll")) (get-structured-python type))
               (if (equal? n #\nul) (string->symbol "") (string->symbol n))
    ...
    [(hash-table ('nodetype "GeneratorExp")
     ...
     (match (first gen)
       [(hash-table ('nodetype "comprehension")
                    ...
                    ('iter iterable)
                    ('ifs ignore))
        (PyCmp (get-structured-python t) (get-structured-python iterable) (get-structured-python func))]
       [_ (error 'parse "invalid comprehension format")])]
    [(hash-table ('nodetype "Global")
                 ('names names))
     (PyGlobal (string->symbol (first names)))]
